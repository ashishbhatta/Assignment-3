...
diary('output.txt');  % Start recording and save to 'output.txt'
diary on;             % Enable recording




bounds = ([[0, (222710 - 200000)]; [0, (222710 - 200000)]]);
t_init = 100 ;
t_final = 0.01;
cooling_rate = 0.99;
%cooling_rates = [0.9999, 0.98, 0.95, 0.92, 0.8];
cooling_rates = [0.99];
...
for i = 1:length(cooling_rates)
    ...
    %[final_solution, final_quality, temperatures ] = simulated_annealing(bounds, t_init, t_final, cooling_rate);
...
function [iterations, current_solutions, current_params, current_solution, temperatures] = ... 
    ...
    while (t_current > t_final) 
        ...
        perturbed_solution = power_factory(current_params);
...
% function current_solution = obj_func(parameters)
%     % Compute the objective function value
%     %current_solution = sum((parameters-2).^2);
%     current_solution = (parameters(1,1)-4)^2 + (parameters(1,2)-5)^2;
% end
